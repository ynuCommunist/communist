...
public class AdminFindServiceImpl implements AdminFindService {
    ...
    @Override
    public PersonData getById(String finding) {
    ...
    @Override
    public Page<PersonData> getListByNation(Integer pageNum, Integer pageSize, String finding, boolean isAccurate, String sort, String sortBy){
        Pageable pageable = PageRequest.of(pageNum, pageSize, Sort.Direction.valueOf(sort), sortBy);
        Page<PersonData> page;

        if(isAccurate){
            page = findRepository.findByNation(finding,pageable);
        }else {
            page = findRepository.findByNationLike("%"+finding+"%",pageable);
        }

        if (!page.hasContent()){
            throw new MyException(ResultEnum.NATION_NOT_EXIST);
        }
